...
Validate validation message is appear if title is empty
    Open create notification page
    Fill title field in form notification    ${EMPTY}
...
Validate validation message is appear if content is empty
    Open create notification page
    Fill content field in form notification    ${EMPTY}
    Click save button in form notification
...
Validate Message Appears When Create New Notification is Successful
    ${UNIQUE_VALUE}    Generate timestamp
    Open create notification page
    Fill title field in form notification    create_${UNIQUE_VALUE}
    Fill content field in form notification    create_${UNIQUE_VALUE}
    ...
    Upload home screen image    ${IMAGE_100KB}
    Click save button in form notification
    Validate modal message is    ${MODAL_MSG_SUCCESS_CREATE_TEXT}
    Save Notification Title To Yaml    create_${UNIQUE_VALUE}